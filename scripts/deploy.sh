...
# VALUES
#
NAMESPACE=$(yq eval '.cron-chart.namespace' "$VALUES_FILE_NAME")
...
# 🔒 KV
#
KEYVAULT_NAME=$(yq eval '.cron-chart.keyvault.name' "$VALUES_FILE_NAME")
...
helm template . -f "$VALUES_FILE_NAME" \
  --set cron-chart.azure.workloadIdentityClientId="$CLIENT_ID" \
  --debug
